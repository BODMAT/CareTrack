        ...
        <section className="myContainer transitioned text-[var(--color-text)] py-15 border-t-2">
            ...
            <div className="flex flex-wrap gap-7 ">
                {animals.map((animal: IAnimal) => (
                    <div className="flex-1/5 max-xl:flex-1/4 max-lg:flex-1/3 max-md:flex-1/2 max-sm:flex-[1_1_100%] p-4 rounded-xl bg-[var(--color-card)] shadow border flex justify-between items-start">
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => checkBeforeDelete(() => deleteAnimal.mutate(animal.id))} className="cursor-pointer transitioned hover:scale-90">
                                <img className="w-6" src={CrossSVG} alt="cross" />
                            </button>
                            <button onClick={() => { open("Форма для редагування тварини", <ModalAnimal id={animal.id} />, true) }} className="cursor-pointer transitioned hover:scale-90">
