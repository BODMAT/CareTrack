    ...
    return (
        <motion.section
            ...
            <div className="flex gap-2 max-[450px]:flex-col">
                <button
                    onClick={() => { user ? open("Форма для додавання догляду", <p className="mb-5">...</p>) : open("Notification", <p className="mb-5">Спочатку авторизуйтеся</p>) }}
                ...
                <button
                    onClick={() => { user ? open("Форма для додавання наряду", <p className="mb-5">...</p>) : open("Notification", <p className="mb-5">Спочатку авторизуйтеся</p>) }}
                    className="text-[var(--color-text)] fontText px-8 py-5 rounded-2xl bg-[image:var(--color-background)] border-2 transition-transform hover:scale-95 hover:shadow-xl cursor-pointer
                ">Додати наряд</button>
            </div>
        </motion.section >
